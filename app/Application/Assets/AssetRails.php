...
final class AssetRails
{
    public function enabled(string $tenantId, string $code, string $operation): array
    {
        ...
        if (! $rail || ! $connection?->enabled || $connection->start_height === null || $connection->next_height === null || ! $rail->deposit_address || ! $company || ! in_array($operation, ['deposit', 'withdrawal'], true) || ! $company->{$operation.'_enabled'} || ($operation === 'deposit' ? $company->minimum_deposit === null : $company->withdrawal_fee === null)) {
