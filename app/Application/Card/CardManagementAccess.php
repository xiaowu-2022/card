...
final readonly class CardManagementAccess
{
    ...
    public function card(string $tenantId, string $userId, string $cardId, bool $lock = false, bool $requireActive = true): UserCard
    {
        ...
        if ($card->provider !== 'PHOTONPAY' || $card->card_currency !== 'USD' || ! app(CardProductProviderRouter::class)->forCard($card)->available()) {
