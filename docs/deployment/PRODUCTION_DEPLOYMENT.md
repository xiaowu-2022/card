...
## 2026-09-24 PhotonPay 多账号发布

参见 [PHOTONPAY_ACCOUNTS.md](../architecture/PHOTONPAY_ACCOUNTS.md)。先在维护窗口备份并部署迁移文件，运行数据库迁移，再核对账号与永久 BIN 占用，然后发布完整应用/前端并重载 PHP-FPM/常驻 PHP 进程。禁止旧应用在迁移期间继续新增产品或更改连接。

1. 发布前只读运行 `php artisan cards:accounts-preflight`；迁移前返回“尚未迁移”是预期结果，先查看 `.env` 依赖产品和重复 BIN。
2. 运行 `php artisan migrate --force`。迁移不调用 PhotonPay、不创建卡、不转移资金。
3. 重跑预检，保留 `522105`、`53493435` 历史重复，核对永久占用的在用/归档归属。冲突 BIN 不允许新配置，不能删除记录来绕过。
