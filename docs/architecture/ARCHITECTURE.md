...
Phase 5/5.1 completes and integrity-hardens idempotent Wallet Top-up initiation, Payment Provider integration, signed event processing, query recovery, and exact Ledger settlement. KYC approval remains side-effect free; an eligible User explicitly activates the default-asset Wallet in a separate Application Action. Activation provisions zero-balance Accounts without a synthetic Entry. V1 aligns Deposit and Tenant default assets and freezes the asset after financial Account creation. Tenant Admin payment views are read-only; Users may activate a Wallet and start a configured Top-up. The project intentionally stops before withdrawal, security-deposit payment/refund, Card Product, card issue/load, real providers, agents, commission, and SaaS billing.
