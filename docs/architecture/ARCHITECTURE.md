...
Card business code depends on `CardProviderInterface`. `MockCardProvider` and the real `PhotonPayCardProvider` exercise the same Cardholder, product-validation, issue, safe-read, and query paths. An external call is made only after local intent/hold commits, then settled or released in a new transaction. A timeout is UNKNOWN.
