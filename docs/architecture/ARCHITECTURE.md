...
Phase 5 introduces an independent Payment Domain: `WalletTopupOrder` is business state, `PaymentProviderTransaction/Event` is integration state, and Ledger remains financial truth. The initiation saga commits intent before external I/O. Only signed Webhooks or trusted Provider queries produce PAID; only a deterministic `LedgerWriter` event produces CREDITED. Persisted events and PAID Orders plus `payments:recover` close queue-dispatch gaps. Payment adapters do not import or mutate Ledger, and webhook Tenant identity is resolved from trusted Provider mappings.

Phase 5.1 hardens that boundary without adding a new capability. CREDITED is database-enforced and irreversible, Provider identities/events are unambiguous and immutable, initiation leases close pre-call/response-loss crash windows with the same external request id, and centralized monotonic transition rules serialize query/webhook/refund/credit races. `payments:reconcile` verifies every credited Order against its exact sealed Ledger plan.
...
Phase 7 completes lean manual USDT-TRC20 withdrawal with immediate hold, scoped Admin review, protected destination reveal, exact chain verification, and deterministic settle/release events. KYC approval remains side-effect free; an eligible User explicitly activates the default-asset Wallet. V1 still aligns Deposit and Tenant default assets and freezes the asset after financial Account creation. The project intentionally stops before automated/custodial payout, monitored crypto deposit, bank/fiat or multi-chain rails, Security Deposit refund, Card Product, card issue/load, real providers, agents, commission, and SaaS billing.
