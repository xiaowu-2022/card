...
Phase 9 adds the lean platform-owned PhotonPay regular-card product catalog and Tenant sales configuration. It is configuration/read UI only: no Provider call, user card, issue/load, card balance, or financial mutation exists. KYC approval remains side-effect free; an eligible User explicitly activates the default-asset Wallet. The project intentionally stops before PhotonPay integration, Cardholder, card issue/load, Security Deposit refund, agents, commission, and SaaS billing.
