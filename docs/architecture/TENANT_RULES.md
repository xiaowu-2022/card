...
- Top-up browser requests inherit Host-resolved Tenant Context and scope both Wallet and Order by Tenant+User. Webhooks ignore Host and submitted Tenant identifiers; only a verified Provider request/transaction mapping derives Tenant. Payment jobs carry explicit trusted Tenant and Event/Order/Transaction UUIDs.
- Payment recovery derives callback hosts only from persisted ACTIVE Tenant Domains, passes explicit Tenant/resource ids to jobs, and clears worker TenantContext after every execution. Provider request/transaction mappings must be unique and unambiguous across Tenants within each Provider.
- V1 has one stable Tenant financial asset. Security Deposit asset must equal `tenant.default_asset`; once a Wallet or Ledger Account exists, ordinary settings/direct updates cannot change that default asset. A future change requires a dedicated asset-migration or multi-asset architecture.
- Wallet activation and User suspend/reactivate actions use the same Tenant -> User row-lock order. Audit foreign keys must never cause a User-first action to wait backward on a Tenant held by an activation flow.
