...
Phase 7 withdrawal is fixed to USDT on TRON and has no fee or FX. Creation atomically moves the exact amount from USER_AVAILABLE to USER_WITHDRAWAL_HOLD. User cancellation or an eligible Admin rejection moves the exact amount back. Only exact confirmed on-chain evidence settles USER_WITHDRAWAL_HOLD to TENANT_WITHDRAWAL_CLEARING. Approval, address reveal, transaction submission, pending confirmation, invalid chain evidence, and gateway failure never move money. No administrator can manually settle, release, or adjust a hold.

Phase 8 shared-address top-up accepts a requested USDT decimal string with at most two fractional digits and adds one exact `0.01..0.99` identifier using decimal arithmetic. The identifier is not a fee: `amount`, expected blockchain transfer, and eventual Wallet credit are identical. Detection and pending confirmations never affect Ledger. Only exact trusted confirmation establishes PAID, and the unchanged Phase 5 settlement posts `TENANT_TOPUP_CLEARING -expected_amount` and `USER_AVAILABLE +expected_amount` once.
