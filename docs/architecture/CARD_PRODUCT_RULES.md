...
Future Wallet debits remain Ledger-authoritative. Future PhotonPay card balance is Provider-authoritative; any local card balance is a cached read-model mirror only. Once a real card references a product, provider reference, currency, and card type cannot be changed silently; changing identity requires a new product. Phase 9 deliberately creates no User Card, Cardholder, Card Issue, Card Load, PAN/CVV, transaction, balance-sync, credential, or Provider API capability.
